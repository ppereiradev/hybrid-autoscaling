...
USAGE="Usage: `basename $0` [-a load balancer address] [-p load balancer root password] [-i sample collection interval in seconds] [-c cpu load] [-m ram memory] [-b bandwidth] [-h help]"
while getopts ":a:p:cmbi:h" opt; do
  case $opt in
    a) LBDHCPADDRESS="$OPTARG"
    ;;
    p) PASSWORD="$OPTARG"
    ;;
    c) RESOURCE="c"
    ...
    ;;
    \?) 
		echo "`basename $0`: option '-$OPTARG' not recognised." >&2
		echo "`basename $0`: '-h' gives usage information."
	;;
  esac
done
            ...
            then
                    ...
                    then
                            ...
                            then
                                    ...
                                    #LASTIP[$((LINES+1))]=$(sshpass -p $PASSWORD ssh -o StrictHostKeyChecking=no $LBDHCPADDRESS -l root "cat /var/lib/dhcp/dhcpd.leases | grep -B 8 webserver | tail -9 | head -1 | awk '{ print \$2 }'")
                                    ...
                                    sshpass -p $PASSWORD ssh -o StrictHostKeyChecking=no $LBDHCPADDRESS -l root sed -i "5i\ server\ ${LASTIP[$((LINES+1))]}\;" /etc/nginx/sites-available/default

                                    sshpass -p $PASSWORD ssh -o StrictHostKeyChecking=no $LBDHCPADDRESS -l root service nginx restart

                                    #apagando logs gigantes
                                    sshpass -p $PASSWORD ssh -o StrictHostKeyChecking=no $LBDHCPADDRESS -l root rm -rf /var/log/nginx/access.log*        

                                    #apagando logs gigantes vm1
                                    sshpass -p $PASSWORD ssh -o StrictHostKeyChecking=no 192.168.0.211 -l root rm -rf /var/log/apache2/access.log*                             
                            ...
                            then
                                    ...
                                    then
                                            ...
                                            sshpass -p $PASSWORD ssh -o StrictHostKeyChecking=no $LBDHCPADDRESS -l root sed -i "/${LASTIP[$LINES]}/d" /etc/nginx/sites-available/default
                                            sshpass -p $PASSWORD ssh -o StrictHostKeyChecking=no $LBDHCPADDRESS -l root service nginx restart
